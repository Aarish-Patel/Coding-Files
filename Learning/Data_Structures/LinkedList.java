...
class LinkedList<T extends Comparable<T>> implements Cloneable
{
    ...
    public LinkedList[] splitmid()
    {
        int mid = length/2;
        LinkedList<T> list1 = this.cloneLinkedList();
        LinkedList<T> list2 = new LinkedList<>();
        while(list2.length <= mid)
        {
            list2.insert(list1.pop(mid));
        }
        LinkedList[] split = {list1, list2};
        return split;
    }
    public LinkedList[] alternatingsplit()
    {
        int odd = 1;
        double test_cond = Math.ceil((length + 1)/2.0);
        LinkedList<T> evenlist = this.cloneLinkedList();
        LinkedList<T> oddlist = new LinkedList<>();
        while(odd != test_cond)
        {
            oddlist.insert(evenlist.pop(odd));
            odd += 1;
        }
        LinkedList[] split = {evenlist, oddlist};
        return split;
    }
    public LinkedList[] split(int m, int n)
    {
        double factor = (double)(length)/(m + n);
        int temp = (int)(m * factor);
        int relm = (int)(m * factor);
        LinkedList<T> list1 = this.cloneLinkedList();
        LinkedList<T> list2 = new LinkedList<>();
        ...
        while(node != null)
        {
            list2.insert(list1.pop(relm));
            node = node.next;
        }
        LinkedList[] split = {list1, list2};
        return split;
    }
    public LinkedList<T> cloneLinkedList()// making my own clone method although object has a clone method it throws exception and is irritating
    {
        LinkedList<T> cloned= new LinkedList<>();
        for(int i = 0; i < this.length; i ++)
        {
            cloned.insert(this.get(i));
        }
        return cloned;
    }
    ...
    public T get(int index)
    {
        ...
        if(index >= length)
        {
            System.err.println("Mentioned index is greater than list length : NullPointerException Data_Structure.LinkedList.get");
            System.exit(0);
        }
    ...
    public void set(int index, T ndata)
    {
        ...
        if(index >= length)
        {
            System.err.println("Mentioned index is greater than list length : NullPointerException in Data_Structure.LinkedList.set");
            System.exit(0);
        }
        for(int i = 0; i < index; i ++)
        {
